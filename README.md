...
集成 Micrometer + Prometheus，暴露 `/metrics` 端点供 Prometheus 抓取。

**功能特点：**
- 📈 **系统指标** — JVM 内存、CPU 使用率、进程信息
- 🤖 **模型指标** — 已加载模型数、加载/卸载计数、加载耗时
- 📡 **请求指标** — 请求总数、错误数、Token 消耗、请求耗时
- 🔒 **安全控制** — `/metrics/config` 仅允许 localhost 访问，支持动态启用/禁用

**端点：**
| 端点 | 方法 | 说明 |
|------|------|------|
| `/metrics` | GET | Prometheus 格式指标（需启用） |
| `/metrics/config` | GET | 查看当前配置（仅 localhost） |
| `/metrics/config?enabled=true/false` | GET | 动态启用/禁用（仅 localhost） |

### 🔥 智能模型预热 (Smart Warmup)

JIT 加载模型后自动发送预热请求，消除首次推理的冷启动延迟。

**配置项（在 `application.json` 的 `jit.warmup` 中）：**
| 配置 | 默认值 | 说明 |
|------|--------|------|
| `enabled` | `false` | 是否启用预热 |
| `prompt` | `"Hello"` | 预热 prompt |
| `maxTokens` | `8` | 预热生成长度（token 数） |
| `timeout` | `30000` | 预热超时（毫秒） |

### 📋 请求队列与优先级

当模型正在加载时，将请求加入队列等待而非立即返回失败。支持三级优先级（high/normal/low）。

**配置项（在 `application.json` 的 `jit.queue` 中）：**
| 配置 | 默认值 | 说明 |
|------|--------|------|
| `enabled` | `true` | 是否启用请求队列 |
| `maxQueueSize` | `100` | 每个模型的最大队列长度 |
| `defaultTimeout` | `300000` | 队列等待超时（毫秒） |
| `priority.enabled` | `false` | 是否启用优先级 |
| `priority.header` | `"X-Request-Priority"` | 优先级识别头 |

**优先级值映射：**
| 值 | 优先级 |
|----|--------|
| `high` / `1` / `urgent` | HIGH |
| `low` / `-1` / `background` | LOW |
| 其他 / 未设置 | NORMAL |
...
| 端口 | 用途 |
|------|------|
| 8080 | WebUI + OpenAI/Anthropic API + WebSocket + /metrics |
...
### v2026-05-31
- **新功能**：Prometheus 指标监控，暴露 `/metrics` 端点（Micrometer + Prometheus 格式），含系统/模型/请求三类指标
- **新功能**：智能模型预热（Smart Warmup），JIT 加载后自动发送预热请求消除冷启动延迟
- **新功能**：请求队列与优先级系统，模型加载中请求排队等待，支持 high/normal/low 三级优先级
- **安全修复（P0）**：`/metrics/config` 端点增加 localhost 访问控制（MetricsController）
- **安全修复（P0）**：查询参数解析使用白名单验证，防止参数注入（MetricsController）
- **安全修复（P0）**：`QueuedRequest` 存储 HTTP method，防止 HTTP/2 → HTTP/1.1 回放丢失（RequestQueueManager）
- **Bug 修复（P0）**：`failAll()` 通过 OpenAIService 发送 HTTP 错误响应而非静默丢弃（RequestQueueManager）
- **Bug 修复（P0）**：`drainAll()` 持有 `readyLock` 防止竞态条件，移除死代码（RequestQueueManager）
- **修复**：GitHub Actions workflow 使用 RELEASE_TOKEN (PAT) 代替 GITHUB_TOKEN，tag 触发规则改为 `v*`
- **修复**：同步 pom.xml 依赖版本，补充 micrometer/prometheus JAR 到 lib/

### v2026-05-30
- **安全修复（P0）**：`/metrics/config` 端点增加 localhost 访问控制，仅允许本机访问（MetricsController）
- **安全修复（P0）**：查询参数解析使用 `ParamTool.getQueryParam()` 白名单验证，防止参数注入（MetricsController）
- **安全修复（P0）**：`QueuedRequest` 存储 HTTP method，避免 HTTP/2 `:method` 伪头在 HTTP/1.1 回放时丢失（RequestQueueManager）
- **Bug 修复（P0）**：`failAll()` 现在通过 OpenAIService 向活跃通道发送 HTTP 错误响应，而非静默丢弃请求（RequestQueueManager）
- **Bug 修复（P0）**：`drainAll()` 持有 `readyLock` 防止竞态条件，移除死代码 `clear()` 和 `processQueue()`（RequestQueueManager）

### v2026-05-29
- **安全修复**：SSL 证书验证现在正确遵守 `ssl.verify` 配置（NettyHttpUtils + AnthropicService）
- **安全修复**：MCP 文件工具路径遍历漏洞已修复
- **Bug 修复**：thinking_budget_tokens 参数现在生效
- **Bug 修复**：SimpleDateFormat 线程安全问题已修复
- **上游同步**：合并 upstream/master 10 commits
